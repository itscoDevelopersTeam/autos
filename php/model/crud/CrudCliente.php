...
require_once('Conexion.php');
	...
	class CrudCliente{
		...
		public function insert( $cliente ){
			...
			$insert=$db->prepare( 'INSERT INTO CLIENTES values( :id_cliente, :nombre, :direccion, :telefono, :email )' );
			...
			$insert->bindValue('email', $cliente->get_email());
			$insert->execute();
		}
		...
		public function update( $cliente ){
			$db = DataBase::conectar();
			$actualizar = $db->prepare('UPDATE CLIENTES SET NOMBRE=:nombre, DIRECCION=:direccion, TELEFONO=:telefono, EMAIL=:email  WHERE ID_CLIENTE=:id_cliente');
			...
			$actualizar->bindValue('email', $cliente->get_email());
			$actualizar->execute();
		}
		...
		public function select_all(){
			...
			foreach($select->fetchAll() as $cliente){
				...
				$myCliente->set_email($cliente['EMAIL']);
